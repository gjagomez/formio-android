        ...
        actionBar.addView(MaterialButton(context).apply {
            ...
            setOnClickListener {
                if (sigView.isSignatureEmpty()) {
                    Toast.makeText(context, "Por favor dibuje su firma primero", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                saveSignatureToGallery(sigView.getBitmap())
                onSaved(sigView.getBitmapBase64())
                dialog.dismiss()
            }
        })
