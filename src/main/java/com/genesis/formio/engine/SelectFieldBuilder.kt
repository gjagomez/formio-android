...
class SelectFieldBuilder(private val context: Context) {
    ...
    fun build(
        ...
        // ── Options ────────────────────────────────────────────────────────────
        val options: List<SelectOption> = when (component.dataSrc) {
        ...
        fun onSelected(selected: SelectOption) {
            ...
            formData[component.key] = selected.value
            onChange(component.key, selected.value)
        ...
        card.setOnClickListener {
            ...
            if (component.searchEnabled) {
                showSearchSheet(component, options, formData[component.key]?.toString(), ::onSelected) {
                    card.strokeColor = context.getColor(R.color.input_border)
                }
            } else {
