...
class FormWizardActivity : AppCompatActivity(), PhotoCaptureLauncher {
    ...
    private val cameraPermissionLauncher = registerForActivityResult(
        ...
        else Toast.makeText(this, R.string.formio_msg_permiso_camara, Toast.LENGTH_SHORT).show()
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val schemaJson = intent.getStringExtra(EXTRA_SCHEMA_JSON) ?: run { finish(); return }
        ...
        viewModel.pages.observe(this) { pages ->
            try { setupPager(pages); setupTabs(pages) } catch (_: Exception) {}
        }
    ...
    private fun setupPager(pages: List<FormWizardViewModel.FormPage>) {
        ...
        binding.btnNext.setOnClickListener {
            ...
            val fragment = pagerAdapter.getFragment(currentIndex) as? FormPageFragment
        ...
        binding.btnPrev.setOnClickListener {
            val currentIndex = binding.viewPager.currentItem
            if (currentIndex > 0) {
                (pagerAdapter.getFragment(currentIndex) as? FormPageFragment)?.clearErrors()
                binding.viewPager.currentItem = currentIndex - 1
            }
        }
    }
    ...
    private fun updateButtons(current: Int, total: Int) {
        binding.btnPrev.visibility = if (current > 0) View.VISIBLE else View.GONE
        binding.btnNext.text = if (current == total - 1) getString(R.string.formio_btn_guardar)
    ...
    private fun doLaunchCamera() {
        try {
            ...
            Toast.makeText(this, R.string.formio_msg_sin_camara, Toast.LENGTH_SHORT).show()
